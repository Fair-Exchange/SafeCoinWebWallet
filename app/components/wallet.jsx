...
class ZWalletGenerator extends React.Component {
  ...
  handlePasswordPhrase(e){
    ...
    var wifHash = this.props.settings.useTestNet ? safejs.config.testnet.wif : safejs.config.mainnet.wif

    var pk = safejs.address.mkPrivKey(e.target.value)
    var pkwif = safejs.address.privKeyToWIF(pk, true, wifHash)
          ...
          <FormGroup row>
            <Col>
              ...
              <FormText color="muted">
                For Windows, it should be in %APPDATA%/Safecoin<br/>
                For Mac, it should be in Library/Application Support/Safecoin<br/>
					 For Linux, it should be in ~/.safecoin
              </FormText>
            </Col>
          </FormGroup>
        </Form>
  ...
  render () {
    return (
      <Modal isOpen={this.props.settings.showSettings} toggle={this.props.toggleModalSettings}>
        <ModalHeader toggle={this.props.toggleShowSettings}>SafeCoin Web Wallet Settings</ModalHeader>
        <ModalBody>
...
class ZSendZEN extends React.Component {
          ...
          .then((infoResp) => {
            ...
            axios.get(blockHashURL)
              .then((responseBhash) => {
                ...
                let network = bitgo.networks['safecoin']
                var keyPair = bitgo.ECPair.fromWIF(senderPrivateKey,network)
                var txb = new bitgo.TransactionBuilder(network)

                if (infoData.info.blocks  >= 547422) { // Sapling active
  ...
  render() {
    ...
    if (this.state.sendProgress === 100){
      ...
      zenTxLink = (
        <Alert color="success">
        <strong>SAFE successfully sent!</strong> <a href={zentx} target="_blank">Click here to view your transaction</a>
        </Alert>
      )
    }
    ...
    return (
      <Row>
        <Col>
          <Card>
            <CardBody>
              <Alert color="danger">ALWAYS VALIDATE YOUR DESTINATION ADDRESS BY SENDING SMALL AMOUNTS OF SAFE FIRST</Alert>
              <InputGroup>
...
class ZWalletTabs extends React.Component {
  ...
  savePrivateKeys(){
    ...
    var fileStr = '# Wallet dump created by SafeCoinWebWallet ' + pjson.version + '\n'
    ...
    FileSaver.saveAs(pkBlob, now + '_SafeCoin_private_keys.txt')
  }
  ...
  handleUnlockPrivateKeys(){
    ...
    try{
      ...
      function _privKeyToAddr(pk, compressPubKey, useTestNet){
        ...
        if (pk.length !== 64){
          pk = safejs.address.WIFToPrivKey(pk)
        }

        // Convert public key to public address
        const pubKey = safejs.address.privKeyToPubKey(pk, compressPubKey)

        // Testnet or nah
        const pubKeyHash = useTestNet ? safejs.config.testnet.pubKeyHash : safejs.config.mainnet.pubKeyHash
        const publicAddr = safejs.address.pubKeyToAddr(pubKey, pubKeyHash)

        return publicAddr
      }

      for (var i = 0; i < this.state.privateKeys.length; i++){
        const pubKeyHash = this.state.settings.useTestNet ? safejs.config.testnet.wif : safejs.config.mainnet.wif
        ...
        if (c_pk.length !== 64){
          c_pk_wif = c_pk
          c_pk = safejs.address.WIFToPrivKey(c_pk)
        }
        else{
          c_pk_wif = safejs.address.privKeyToWIF(c_pk)
        }

        var c_pk_wif = safejs.address.privKeyToWIF(c_pk, true, pubKeyHash)
  ...
  render() {
    return (
      <Container>
        <Row>
          <Col>
            <h1 className='display-6'>SafeCoin Web Wallet&nbsp;
